...
func main() {
	...
	watcher,err := config.NewConfigWatcher(pingerHost,pingerPort, systemDomain, loadBalancerPort)
	if err != nil {
		panic(err)
	}
	mcpServer := server.New(&source.Options{Watcher:watcher},&server.AllowAllChecker{})
